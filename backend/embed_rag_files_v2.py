...
def main():
    ...
    print("=" * 60)
    print("RAG Files Embedding Script (v2)")
    print("=" * 60)
    ...
    try:
        ...
        for i in range(0, len(documents), batch_size):
            ...
            print(f"Adding batch {batch_num} ({batch_end - i} documents)...", end=" ", flush=True)
        ...
        print("=" * 60)
        print(f"Total documents embedded: {total_added}")
        ...
        if collection:
            ...
            # Try a sample query
            if count > 0:
                results = chroma.query(collection_name, ["steel rebar"], n_results=3)
                print(f"✓ Sample query successful - found {len(results['documents'][0])} results")
